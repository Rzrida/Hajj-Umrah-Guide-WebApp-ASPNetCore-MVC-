...

{
    public class AddComment : Controller
    {
        ...
        {
            List<CommentsModel> ls4 = new List<CommentsModel>();
        ...
        [HttpPost]
        public IActionResult Comment(CommentsModel model)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                string q = "INSERT INTO Comments (descs, stepId, userid)  " +
                           "VALUES (@Desc, @StepId, @userId )";
        ...
        public IActionResult DuaRequests(int stepid) // Pass stepid as a parameter
        {
            ...
            List<CommentsModel> Prayers = GeRequestedPrayerFromDatabase(stepid);
            return View(Prayers);
        }

        private List<CommentsModel> GeRequestedPrayerFromDatabase(int stepid)
        {
            byte[] userIdBytes = HttpContext.Session.Get("LoggedUserId");
            int userId = 0;
            if (userIdBytes != null)
            ...
            {
                con.Open();
                string q = " SELECT * FROM Comments WHERE userid =  @userId"; // Filter by stepid
                using (SqlCommand cmd = new SqlCommand(q, con))
                {
